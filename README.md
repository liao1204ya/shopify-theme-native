...
当前版本：**v1.1.0**。本版修复设计稿还原可能导致替换原有 header/footer、改动全站字体和布局的范围判断问题。
...
- **最高默认优先级：跟随目标主题字体、布局、容器、间距、断点、配色及动画；仅在用户明确要求时修改对应项。**
- **保留目标主题已有公共 header/footer；忽略所有设计稿中的 header/footer，包括首页稿。**
- 设计稿仅用于授权范围内的内容区域还原；高还原目标不覆盖上述规则。
